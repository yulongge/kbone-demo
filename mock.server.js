...
app.all('*', function(req, res, next) {  
    res.header("Access-Control-Allow-Origin", `http://${serverConfig.getHost(process)}:8080`);
...
function runserver(port, dir) {
	...
	}).on('listening', e=>console.log(`server run at http://${host}:${port} (${dir}), prefix is ${serverConfig.mock_prefix}`));
}
